...
OTA_UPDATE_SERVER ?= 192.168.1.2
OTA_UPDATE_PORT ?= 8080
OTA_UPDATE_PATH ?= "/ota-updates/sonoff-th"
...
sonoff_th_slot_0 : LINKER_SCRIPTS = $(PROGRAM_DIR)ld/slot_0.ld $(ROOT)ld/rom.ld
sonoff_th_slot_0 : override PROGRAM = sonoff_th_slot_0

sonoff_th_slot_1 : LINKER_SCRIPTS = $(PROGRAM_DIR)ld/slot_1.ld $(ROOT)ld/rom.ld
sonoff_th_slot_1 : override PROGRAM = sonoff_th_slot_1
...
sonoff_th_slot_0: $(WHOLE_ARCHIVES) $(COMPONENT_ARS) $(BUILD_DIR)libgcc.a $(BUILD_DIR)libc.a $(SDK_PROCESSED_LIBS) $(LINKER_SCRIPTS)
...
sonoff_th_slot_1: $(WHOLE_ARCHIVES) $(COMPONENT_ARS) $(BUILD_DIR)libgcc.a $(BUILD_DIR)libc.a $(SDK_PROCESSED_LIBS) $(LINKER_SCRIPTS)
	$(vecho) "Linking OTA firmware for slot 1"
	$(Q) $(LD) $(LDFLAGS) -Wl,--whole-archive $(WHOLE_ARCHIVES) -Wl,--no-whole-archive -Wl,--start-group $(COMPONENT_ARS) $(BUILD_DIR)libgcc.a $(BUILD_DIR)libc.a $(LIB_ARGS) $(SDK_LIB_ARGS) -Wl,--end-group -o $(BUILD_DIR)$@.out

ota-images: $(OTA_DIR) sonoff_th_slot_0 sonoff_th_slot_1
	$(vecho) "Building OTA firmware images"
	$(Q) $(ESPTOOL2) -quiet -bin -boot2 $(BUILD_DIR)sonoff_th_slot_0.out $(OTA_DIR)$(OTA_UPDATE_FIRMWARE_NAME)_0.bin .text .data .rodata
	$(Q) $(OPENSSL) sha256 < $(OTA_DIR)$(OTA_UPDATE_FIRMWARE_NAME)_0.bin > $(OTA_DIR)$(OTA_UPDATE_FIRMWARE_NAME)_0.sha256
	$(Q) $(ESPTOOL2) -quiet -bin -boot2 $(BUILD_DIR)sonoff_th_slot_1.out $(OTA_DIR)$(OTA_UPDATE_FIRMWARE_NAME)_1.bin .text .data .rodata
